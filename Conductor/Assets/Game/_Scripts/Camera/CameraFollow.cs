...
public class CameraFollow : MonoBehaviour
{
    ...
    void SmoothFollow()
    {
        ...
        if (transform.position.x >= followTarget.position.x + followDistance)   // Camera is to the right of player's follow distance
        {
            nextFramePosition = Vector3.Lerp(transform.position, new Vector3(followTarget.position.x + followDistance, followTarget.position.y, followTarget.position.z), followSpeed * Time.deltaTime);
        }
        else if(transform.position.x <= followTarget.position.x - followDistance)   // Camera is to the left of player's follow distance
        {
            nextFramePosition = Vector3.Lerp(transform.position, new Vector3(followTarget.position.x - followDistance, followTarget.position.y, followTarget.position.z), followSpeed * Time.deltaTime);
        }
        else        // Camera is within the player's follow distance
        {
            nextFramePosition = Vector3.Lerp(transform.position, new Vector3(transform.position.x, followTarget.position.y, transform.position.z), followSpeed * Time.deltaTime);
        }
