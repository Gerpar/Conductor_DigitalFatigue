...
public class PlayerMove : MonoBehaviour 
{
    ...
    {
        if(transform.position.x > mainCam.GetComponent<Camera>().ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, -mainCam.transform.position.z)).x)   // Flips the player if they move past the target (Mouse cursor)
        {
